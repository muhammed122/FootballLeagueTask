...
) : FlowUseCase<Int, CompetitionDetailsModel?>(ioDispatcher) {
    override fun execute(parameters: Int): Flow<CompetitionDetailsModel?> =
        flow {
            emit(repository.getCompetitionDetails(parameters))
        }
}